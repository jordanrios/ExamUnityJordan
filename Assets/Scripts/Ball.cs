...
public class Ball : MonoBehaviour
{
    ...
    [SerializeField] private GameObject powerUpLoseLife;
    [SerializeField] private GameObject powerUpSize;

    

    private int contLimitValocity = 0;
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.gameObject.CompareTag("block")) // Si colisionamos con un bloque
        {
            ...
            if (contLimitValocity <= 6)//LIMITAMOS LA VELOCIDAD DE LA PELOTA
            {
            ...
            // Si el aleatorio es menor que 0.2 (20% de probabilidad) generamos un powerUp en la posición de la colisión
                ...
                {
                    Instantiate(powerUpLoseLife, collision.transform.position, Quaternion.identity);
                }
                if (random >= 0.4f && random <= 0.55f)
                {
                    Instantiate(powerUpSize, collision.transform.position, Quaternion.identity);
                }
